...
Router.onBeforeAction( loadEventsPerDay, { only : ['engagements']});

Router.route("/",
	function() {
		this.layout("layout");
		this.render("engagements", {to: "mainPane"});
		this.render('eventDay', {to: "dayTemplate"});
	}, 
	{  name: "engagements" }
);

Router.route("/engagements/:_id",
  function() {
    this.wait(Meteor.subscribe('engagements'));
    this.layout("layout");
    this.render(
      "engagementDetailsCard", { 
        to: "mainPane",
        data : function () {
          return Engagements.findOne({ _id : this.params._id});
        } 
      }
    );
  }, 
  {  name: "engagementDetailCard" }
);
